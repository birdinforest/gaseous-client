...
import GameList from '@app/components/apps/GameList';

const DashboardPage: React.FC = () => {
  const { isDesktop } = useResponsive();

  const dispatch = useAppDispatch();
  const games = useAppSelector((state) => state.games.games); // Access games from the store
  const status = useAppSelector((state) => state.games.status);
  const error = useAppSelector((state) => state.games.error);

  useEffect(() => {
    // Fetch games if the games array is empty and the status is 'idle'
    if (status === 'idle') {
